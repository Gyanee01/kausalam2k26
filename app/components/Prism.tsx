...
const Prism: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    // We don't need depth test or cull face for a fullscreen quad shader
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        const int STEPS = 60; // Reduced for performance
    ...
    const render = (t: number) => {
      ...
      if (animationType === 'hover') {
        ...
        yaw = lerp_val(yaw, targetYaw, INERT);
        pitch = lerp_val(pitch, targetPitch, INERT);
        program.uniforms.uRot.value = setMat3FromEuler(yaw, pitch, roll, rotBuf);
        if (NOISE_IS_ZERO && Math.abs(yaw - targetYaw) < 1e-4) continueRAF = false;
